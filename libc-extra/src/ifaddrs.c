...
static int interpretLink(struct nlmsghdr* p_hdr, struct ifaddrs** p_resultList)
{
    ...
    struct ifaddrs* l_entry = (struct ifaddrs*)malloc(sizeof(struct ifaddrs) + sizeof(int)
...
static int interpretAddr(struct nlmsghdr* p_hdr, struct ifaddrs** p_resultList, int p_numLinks)
{
    ...
    struct ifaddrs* l_entry =
        (struct ifaddrs*)malloc(sizeof(struct ifaddrs) + l_nameSize + l_addrSize);
    if (l_entry == NULL)
        return -1;
    memset(l_entry, 0, sizeof(struct ifaddrs));
        ...
        switch (l_rta->rta_type) {
            ...
            case IFA_LOCAL: {
                ...
                if (l_rta->rta_type == IFA_ADDRESS) {
                    /* Apparently in a point-to-point network IFA_ADDRESS contains the dest address
                     * and IFA_LOCAL contains the local address. */
                    if (l_entry->ifa_addr) {
...
static int interpretAddrs(int p_socket, NetlinkList* p_netlinkList, struct ifaddrs** p_resultList,
                          int p_numLinks)
{
    pid_t l_pid = getpid();
    for (; p_netlinkList; p_netlinkList = p_netlinkList->m_next) {
